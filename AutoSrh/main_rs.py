...
parser.add_argument("--data_path", type=str, default='data/AZ/Electronics.csv')
...
parser.add_argument("--feature_split", type=str, default='[1.0]')
...
def generate_random_dims(max_dim, num_group, min_dim=4):
    dims = []
    for i in range(num_group):
        if max_dim > min_dim:
            min_dim = random.randrange(min_dim, max_dim)
        dims += [min_dim]
    return np.array(dims[::-1])
...
def main():
    ...
    dim_array = generate_random_dims(args.embedding_dim, len(eval(args.feature_split)))
