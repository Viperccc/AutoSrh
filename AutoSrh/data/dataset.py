...
def get_avazu_dataset(data_path):       
    if os.path.isfile(f"{data_path}"):
        print("The dataset has been processed. Reading the cache...")
        with open(f"{data_path}", 'rb') as handle:
            result = pickle.load(handle)
...
def get_cf_dataset(data_path, filter_item=True, test_ratio=0.1, val_ratio=0.2, seed=12):
    ...
    rating_df = pd.read_csv(data_path,header=None)
    ...
    if filter_item:
        ...
        rating_df = rating_df.groupby('movieId').filter(lambda x: len(x) >= 10)
        rating_df = rating_df.groupby('userId').filter(lambda x: len(x) >= 10)
