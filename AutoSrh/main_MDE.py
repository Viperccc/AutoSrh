...
parser.add_argument("--feature_split", type=str, default='[0.1,0.2,0.2,0.2,0.3]')
parser.add_argument("--d_base", type=int, default=16)
...
def generate_p_vector(freqs,splits):
    sum_freqs = []
    last = 0
    for split in splits:
        sum_freqs+=[sum(freqs[last:int(len(freqs)*split+last)])]
        last += int(len(freqs)*split)
    p_vector = [freq/sum(sum_freqs) for freq in sum_freqs]
    return sum_freqs, p_vector


def generate_d_vector(p_vector, d_base, sorted_freqs, alpha, splits):
    B = d_base*len(sorted_freqs)
    Lambda = B*(sum([p**(alpha) for p in p_vector]))**(-1)
    d_vector = [Lambda*p**alpha for p in p_vector]
    d_vector_average = [d/(len(sorted_freqs)*split) for d,split in zip(d_vector,splits)]
    d_vector_average = [int(d) for d in d_vector_average]
    return d_vector_average
...
def main():
    ...
    if args.dataset_type == "ctr":
        ...
        with open("data/criteo/sorted_freqs.pkl", 'rb') as f:
            sorted_freqs = pickle.load(f)
        splits = [0.1,0.1,0.2,0.2,0.2,0.2]
    else:
        train_dataset, val_dataset, test_dataset, num_features = get_cf_dataset(args.data_path)
        with open("data/ml-20m/sorted_freqs.pkl", 'rb') as f:
            sorted_freqs = pickle.load(f)
        splits = [0.1,0.2,0.2,0.2,0.3]
    ...
    _, p_vector = generate_p_vector(sorted_freqs, splits=splits)
    d_vector = generate_d_vector(p_vector, d_base=args.d_base, sorted_freqs=sorted_freqs, alpha=args.alpha,
                                 splits=splits)
    dim_array = np.array(d_vector)
    print(f"dim_array: {dim_array}")
    max_dim = dim_array[0]

    model = Dnis(num_features, max_dim, num_fields=num_fields, num_dim_split=max_dim,
                search_space="embedding_block", normalize=0, model_name=args.model_name,
                feature_split=eval(args.feature_split))
    # model = getattr(base_model, args.model_name)(num_features, args.embedding_dim, num_fields)
    model.alpha = torch.nn.Parameter(torch.tensor(dim_array.astype(np.float32)), requires_grad=False)
    ...
    save_model_and_print_size(model)

def save_model_and_print_size(model):
    torch.save(model.feature_embeddings, f"checkpoint/{args.exp}_{args.model_name}_embeddings.pth")
    file_size = os.path.getsize(f"checkpoint/{args.exp}_{args.model_name}_embeddings.pth")
    print("{} KB, i.e. {} MB used for the original embedding layer.".format(file_size >> 10, file_size >> 20))
    alpha_block_weight = (
            model.alpha.unsqueeze(1).expand(model.num_feature_split, model.num_dim_split) - torch.tensor(
        range(model.num_dim_split)).unsqueeze(0).float().to(model.alpha)).clamp(0, 1)
    alpha_block_mask = alpha_block_weight.repeat_interleave(model.feature_nums.to(model.alpha).long(),
                                                            dim=0).repeat_interleave(
        model.embed_dims.to(model.alpha).long(), dim=1)
    scipy_sparse = coo_matrix((model.feature_embeddings.weight*alpha_block_mask).data.cpu())
    save_npz(f"checkpoint/{args.exp}_{args.model_name}_sparse_embeddings", scipy_sparse, True)
    file_size = os.path.getsize(f"checkpoint/{args.exp}_{args.model_name}_sparse_embeddings.npz")
    print("{} KB, i.e. {} MB used for the sparse embedding layer.".format(file_size >> 10, file_size >> 20))
